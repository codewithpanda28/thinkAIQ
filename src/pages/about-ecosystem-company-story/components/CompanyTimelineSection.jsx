      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
                    ...
                    <div className="flex items-center space-x-3">
                      <div
                        className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                          activeYear === item.year ? item.bgColor : 'bg-muted'
                        }`}
                      >
                        <Icon
                          name={item.icon}
                          size={16}
                          className={activeYear === item.year ? item.color : 'text-muted-foreground'}
                        />
                      </div>
                      <div>
          ...
          <div className="lg:col-span-2">
            <div className="bg-background border border-border rounded-2xl p-8 shadow-brand-md">
              {/* Year Header */}
              <div className="flex items-center space-x-4 mb-6">
                <div className={`w-16 h-16 rounded-2xl flex items-center justify-center ${activeData.bgColor}`}>
                  <Icon name={activeData.icon} size={24} className={activeData.color} />
                </div>
                <div>
                  <div className="text-3xl font-bold text-foreground">{activeData.year}</div>
                  <div className="text-lg font-semibold text-primary">{activeData.title}</div>
                </div>
              </div>

              {/* Description */}
              <p className="text-muted-foreground text-lg mb-8">{activeData.description}</p>
              ...
              <div>
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  {activeData.achievements.map((achievement, index) => (
                    <div
                      key={index}
                      className="flex items-start space-x-3 p-4 bg-muted/30 rounded-lg border border-border"
                    >
              ...
              <div className="mt-8 pt-6 border-t border-border">
                ...
                <div className="progress-bar h-2 bg-muted rounded-full overflow-hidden">
                  <div
                    className="progress-fill bg-primary h-2 rounded-full transition-all duration-500"
